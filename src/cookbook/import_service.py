...
class ImportService:
    ...
    def _run(self) -> None:
        try:
            code = run_scraper(self.root)
            if code == 0:
                self.refresh()
                status = {"status": "succeeded", "message": "Post imported. Refresh the cookbook to view it."}
            elif code == 4:
                status = {"status": "failed", "message": "Could not finish scanning for the oldest post. No post was imported. Please try again later."}
            elif code == 3:
